...
- [ ] **Task 1: Migração `000255_readiness_snapshots_realtime.sql`** (AC: #4, #5)
  - [ ] Criar `sparta/supabase/migrations/000255_readiness_snapshots_realtime.sql`
  - [ ] `ALTER TABLE readiness_snapshots REPLICA IDENTITY FULL;`
  - [ ] `ALTER PUBLICATION supabase_realtime ADD TABLE readiness_snapshots;`
  - [ ] Sem DOWN migration (Realtime é uma configuração não-destrutiva)

- [ ] **Task 2: Função utilitária `isInPreSessionWindow(scheduledAt)`** (AC: #1, #3)
  - [ ] Criar `sparta/src/lib/readiness/realtime-window.ts` (ficheiro novo, importável em testes)
  - [ ] Exportar `isInPreSessionWindow(scheduledAt: string): boolean`
  - [ ] Lógica: `const t = new Date(scheduledAt).getTime(); const now = Date.now(); return now >= t - 4 * 60 * 60 * 1000 && now <= t;`
  - [ ] Exportar `PRE_SESSION_WINDOW_MS = 4 * 60 * 60 * 1000` (constante testável)

- [ ] **Task 3: Atualizar `ReadinessPanel`** (AC: #1, #2, #3)
  - [ ] Editar `sparta/src/components/domain/readiness/readiness-panel.tsx`
  - [ ] Adicionar prop `scheduledAt: string` à interface `ReadinessPanelProps`
  - [ ] Adicionar `players: initialPlayers` prop renomeando o parâmetro existente para `initialPlayers`; criar `useState<PlayerReadinessData[]>(initialPlayers)` para `players`
  - [ ] Adicionar `flashedIds: Set<string>` state: `useState<Set<string>>(new Set())`
  - [ ] Adicionar `isRefreshing: boolean` state: `useState(false)`
  - [ ] Adicionar `inWindow: boolean` state: `useState(() => isInPreSessionWindow(scheduledAt))`
  - [ ] `useEffect` de intervalo (60s) para re-verificar `isInPreSessionWindow` → atualiza `inWindow` state (transição automática quando entra/sai da janela)
  - [ ] `useEffect` de subscrição Realtime (deps: `[sessionId, inWindow]`):
    - Se `!inWindow` → `return` (sem subscrição)
    - `const supabase = createClient()` (import de `@/lib/supabase/client`)
    - `const channel = supabase.channel('readiness-snapshots-' + sessionId).on('postgres_changes', { event: '*', schema: 'public', table: 'readiness_snapshots', filter: 'session_id=eq.' + sessionId }, handleRealtimeEvent).subscribe()`
    - Cleanup: `return () => { void supabase.removeChannel(channel); }`
  - [ ] `handleRealtimeEvent(payload)` — função async dentro do componente:
    - Extrair `updatedPlayerId: string | undefined` de `(payload.new as { player_id?: string })?.player_id ?? (payload.old as { player_id?: string })?.player_id`
    - Chamar `getReadinessPanelData(sessionId)` (Server Action — import direto)
    - Se `result.ok` → `setPlayers(result.data.players)`
    - Se `updatedPlayerId`: `setFlashedIds(prev => new Set(prev).add(updatedPlayerId))`; `setTimeout(() => setFlashedIds(prev => { const n = new Set(prev); n.delete(updatedPlayerId); return n; }), 600)`
  - [ ] `handleManualRefresh` — função async:
    - `setIsRefreshing(true)`
    - Chamar `getReadinessPanelData(sessionId)` → se `result.ok` → `setPlayers(result.data.players)`
    - `setIsRefreshing(false)` (em finally ou sequencial)
  - [ ] Recalcular `readyCount/cautionCount/alertCount` a partir de `players` state (já existente — apenas verificar que usa `players` e não `initialPlayers`)
  - [ ] Passar `flashedIds` para `ReadinessPanelList` e `ReadinessPanelFormation` como nova prop
  - [ ] Passar `onRefresh={handleManualRefresh}` e `isRefreshing` e `inWindow` para `ReadinessPanelHeader`

- [ ] **Task 4: Atualizar `ReadinessPanelHeader`** (AC: #3)
  - [ ] Editar `sparta/src/components/domain/readiness/readiness-panel-header.tsx`
  - [ ] Adicionar props: `onRefresh?: () => void`, `isRefreshing?: boolean`, `inWindow?: boolean`
  - [ ] Quando `!inWindow && onRefresh`: renderizar botão "Atualizar" com `<RefreshCw className={isRefreshing ? 'animate-spin' : ''} />` (import `RefreshCw` de `lucide-react`)
  - [ ] Botão: `disabled={isRefreshing}`, `onClick={onRefresh}`, `aria-label="Atualizar dados de prontidão"`, tamanho `size-4` no icon
  - [ ] Posicionar: à direita dos toggles Lista/Formação (gap no mesmo `flex` group)

- [ ] **Task 5: Atualizar `ReadinessPanelList`** (AC: #2)
  - [ ] Editar `sparta/src/components/domain/readiness/readiness-panel-list.tsx`
  - [ ] Adicionar prop `flashedIds?: Set<string>` à interface `ReadinessPanelListProps`
  - [ ] Remover `sessionId: _` (void pattern) — se for necessário manter para compatibilidade futura, deixar como está mas adicionar `flashedIds`
  - [ ] Passar `flashedIds` para cada `<PositionGroup flashedIds={flashedIds} />`

- [ ] **Task 6: Atualizar `PositionGroup`** (AC: #2)
  - [ ] Editar `sparta/src/components/domain/readiness/position-group.tsx`
  - [ ] Adicionar prop `flashedIds?: Set<string>` à interface `PositionGroupProps`
  - [ ] Passar `flashed={flashedIds?.has(snapshot.player_id) ?? false}` para cada `<PlayerRow>`

- [ ] **Task 7: Atualizar `PlayerRow`** (AC: #2)
  - [ ] Editar `sparta/src/components/domain/readiness/player-row.tsx`
  - [ ] Adicionar prop `flashed?: boolean` à interface `PlayerRowProps`
  - [ ] No `<button>`: adicionar `data-flashed={flashed ? 'true' : undefined}` e classes condicionais:
    ```tsx
    className={`... ${flashed ? 'motion-safe:bg-primary/10 motion-safe:transition-colors motion-safe:duration-200 motion-safe:ease-out' : ''}`}
    ```
  - [ ] `prefers-reduced-motion`: usar `motion-safe:` prefixo do Tailwind (respeita `prefers-reduced-motion: reduce` automaticamente)

- [ ] **Task 8: Atualizar `ReadinessPanelFormation` (Story 5.6)** (AC: #2)
  - [ ] Editar `sparta/src/components/domain/readiness/readiness-panel-formation.tsx`
  - [ ] Adicionar prop `flashedIds?: Set<string>` à interface `ReadinessPanelFormationProps`
  - [ ] Passar `flashed` (com `motion-safe:` classes) nos chips de titulares e banco

- [ ] **Task 9: Atualizar `page.tsx`** (AC: #1)
  - [ ] Editar `sparta/src/app/(staff)/prontidao/page.tsx`
  - [ ] Passar `scheduledAt={scheduledAt}` para `<ReadinessPanel>`

- [ ] **Task 10: Testes** (AC: #5)
  - [ ] Criar `sparta/src/__tests__/readiness/realtime-readiness.test.tsx`
...
- `sparta/src/components/domain/readiness/readiness-panel-formation.tsx` — `flashedIds` prop (Story 5.6)
- `sparta/src/__tests__/app/(staff)/prontidao.test.tsx` — testes adicionais

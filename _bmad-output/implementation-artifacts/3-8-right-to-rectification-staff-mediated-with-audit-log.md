...
- [ ] 1.1 Criar `sparta/supabase/migrations/000190_rectification_requests.sql`
- [ ] 1.2 Criar tabela `rectification_requests`:
  ```sql
...
- [ ] 1.5 pg_cron job `rectification_sla_alert` que corre diariamente às 09:00 UTC:
...
- [ ] 1.6 GRANTs: `GRANT SELECT, INSERT, UPDATE ON rectification_requests TO authenticated; GRANT ALL TO service_role;`

---

### Task 2: Edge Function `send-rectification-sla` (AC #6)

- [ ] 2.1 Criar `sparta/supabase/functions/send-rectification-sla/index.ts`
- [ ] 2.2 Criar `sparta/supabase/functions/send-rectification-sla/deno.json` (igual ao padrão de `erase-cascade`)
- [ ] 2.3 Entry point: **`Deno.serve(handler)`** — NUNCA `export default handler`
- [ ] 2.4 Input: `POST { clubId: string }` — validar UUID com regex `/^[0-9a-f-]{36}$/i`
- [ ] 2.5 Variáveis de ambiente: `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `BREVO_API_KEY`, `BREVO_SENDER_EMAIL`, `APP_URL`
- [ ] 2.6 CORS: `Access-Control-Allow-Origin: ${APP_URL}` (nunca wildcard `*`)
- [ ] 2.7 Criar service-role Supabase client
- [ ] 2.8 Consultar `rectification_requests` com `status='pending'`, `club_id = clubId`, `created_at <= now() - 7 days`
- [ ] 2.9 Se nenhum pedido encontrado: retornar `{ ok: true, sent: false }`
- [ ] 2.10 Obter emails de staff (coach e analyst) do clube via query em `profiles` JOIN `auth.users`
- [ ] 2.11 Enviar email via Brevo:
...
- [ ] 2.12 Retornar `{ ok: true, sent: true, count: N }`

---

### Task 3: Server Action `submitRectificationRequest` (AC #1)

- [ ] 3.1 Adicionar a `sparta/src/lib/actions/data-rights.ts`
- [ ] 3.2 Tipo `RectificationResult = { submitted: true; requestId: string }`
- [ ] 3.3 Tipo `RectificationPayload = { fieldName: 'full_name' | 'birthdate' | 'jersey_num'; requestedValue: string; reason?: string }`
- [ ] 3.4 Função `submitRectificationRequest(payload: RectificationPayload): Promise<Result<RectificationResult, AppError>>`
  - [ ] 3.4.1 Verificar `auth.getUser()` — se não autenticado: `err({ code: 'unauthorized' })`
  - [ ] 3.4.2 Validar `fieldName` contra whitelist: `['full_name', 'birthdate', 'jersey_num']`
  - [ ] 3.4.3 Validar `requestedValue` não vazio, máx 500 chars
  - [ ] 3.4.4 Query `players` com `profile_id = user.id` → obter `playerId`, `club_id`
  - [ ] 3.4.5 Se sem player: `err({ code: 'not_found' })`
  - [ ] 3.4.6 Capturar `current_value` da tabela de origem antes de inserir (ex: `players[fieldName]`)
  - [ ] 3.4.7 Inserir row em `rectification_requests` via service-role (bypass RLS para insert)
  - [ ] 3.4.8 Retornar `ok({ submitted: true, requestId: row.id })`
- [ ] 3.5 Função `submitRectificationRequestByToken(token: string, payload: RectificationPayload): Promise<Result<RectificationResult, AppError>>`
  - [ ] 3.5.1 Re-validar token via `validateToken(token)` (função partilhada existente)
  - [ ] 3.5.2 Extrair `playerId` da resposta de validação
  - [ ] 3.5.3 Obter `club_id` do player
  - [ ] 3.5.4 Capturar `current_value` e inserir igual ao passo 3.4.6–3.4.8

---

### Task 4: Server Actions de staff — aprovação e rejeição (AC #3, #4, #5)

- [ ] 4.1 Adicionar a `sparta/src/lib/actions/data-rights.ts`
- [ ] 4.2 Função `approveRectification(requestId: string): Promise<Result<{ applied: true }, AppError>>`
  - [ ] 4.2.1 Verificar `auth.getUser()` — se não autenticado: `err({ code: 'unauthorized' })`
  - [ ] 4.2.2 Verificar papel: `user_role() in ('coach','analyst')` — se não: `err({ code: 'forbidden' })`
  - [ ] 4.2.3 Carregar request via service-role — verificar `status='pending'` e `club_id = user.club_id()`
  - [ ] 4.2.4 Aplicar alteração na tabela de origem:
  ...
  - [ ] 4.2.5 Actualizar `rectification_requests`: `status='applied'`, `applied_at=now()`, `applied_by=user.id`
  - [ ] 4.2.6 Inserir `audit_logs`: `action='subject.rectified'`, `target_kind='player'`, `target_id=player_id`, `payload = { field: fieldName, before: currentValue, after: requestedValue }`
  - [ ] 4.2.7 Enviar email de notificação ao titular via Brevo (fire-and-forget — não bloqueia resposta)
  - [ ] 4.2.8 Actualizar `notified_at=now()` em `rectification_requests`
  - [ ] 4.2.9 Retornar `ok({ applied: true })`
- [ ] 4.3 Função `rejectRectification(requestId: string, reason: string): Promise<Result<{ rejected: true }, AppError>>`
  - [ ] 4.3.1 Verificar auth + papel (igual a 4.2.1-4.2.3)
  - [ ] 4.3.2 Actualizar `rectification_requests`: `status='rejected'`, `rejected_at=now()`, `rejected_by=user.id`, `reject_reason=reason`
  - [ ] 4.3.3 Enviar email de notificação ao titular via Brevo (fire-and-forget)
  - [ ] 4.3.4 Actualizar `notified_at=now()`
  - [ ] 4.3.5 Retornar `ok({ rejected: true })`

---

### Task 5: Página sujeito `/configuracoes/direitos/retificar` (AC #1, #7)

- [ ] 5.1 Substituir `<PageInDevelopment>` em `sparta/src/app/configuracoes/(subject-rights)/direitos/retificar/page.tsx`
- [ ] 5.2 Server Component: `createServerClient()` → `auth.getUser()`; se não logado: `redirect('/login')`
- [ ] 5.3 Renderizar Client Component `<RetificarAuthClient />`
- [ ] 5.4 Criar `sparta/src/app/configuracoes/(subject-rights)/direitos/retificar/_components/retificar-auth-client.tsx`
  - [ ] `'use client'` — estado: `idle | loading | success | error`
  - [ ] Formulário com:
  ...
  - [ ] Submit: `'Enviar pedido'` — estado loading com spinner + `aria-busy`
  - [ ] Sucesso: `<CalmConfirmation message="Pedido recebido — vais ter resposta em até 7 dias." />`
  - [ ] Erro: mensagem inline `signal/alert` (não toast)
  - [ ] Zero violações axe-core (AC #7)

---

### Task 6: Página sujeito `/direitos/[token]/retificar` (AC #1, #7)

- [ ] 6.1 Substituir `<PageInDevelopment>` em `sparta/src/app/(public)/direitos/[token]/retificar/page.tsx`
- [ ] 6.2 Server Component: re-validar token via `validateToken` (reutilizar pattern de `page.tsx` em Story 3.5/3.7)
- [ ] 6.3 Se inválido/expirado: renderizar `<EmptyState>` (copiar lógica de `/direitos/[token]/apagar/page.tsx`)
- [ ] 6.4 Se válido: renderizar `<RetificarTokenClient token={token} playerName={validation.playerName} />`
- [ ] 6.5 Criar `sparta/src/app/(public)/direitos/[token]/retificar/_components/retificar-token-client.tsx`
  - [ ] Props: `token: string`, `playerName: string`
  - [ ] Mesma lógica que `RetificarAuthClient` mas chama `submitRectificationRequestByToken(token, payload)`
  - [ ] Título: "Retificar dados de {playerName}"

---

### Task 7: Página staff `/configuracoes/direitos-pendentes` (AC #2, #3, #4, #6)

- [ ] 7.1 Criar `sparta/src/app/configuracoes/direitos-pendentes/page.tsx` (Server Component)
- [ ] 7.2 Verificar papel: se `user_role()` não é coach ou analyst → redirect para `/configuracoes`
- [ ] 7.3 Query `rectification_requests` com `status='pending'` e `club_id = user.club_id()` via service-role, ordenado por `created_at ASC`
- [ ] 7.4 Calcular banner de dia 5: se existe pelo menos um pedido com `created_at <= now() - 5 days`, mostrar banner info (signal/info)
  - Copy: "Existem X pedidos de retificação com mais de 5 dias por responder."
- [ ] 7.5 Renderizar Client Component `<PendingRequestsList requests={requests} />`
  - [ ] Criar `sparta/src/app/configuracoes/direitos-pendentes/_components/pending-requests-list.tsx`
  - [ ] `'use client'`
  - [ ] Para cada request:
  ...
  - [ ] Rejeição: pede motivo via `<Dialog>` com `<textarea>` obrigatório
  - [ ] Loading states por request individual (não bloqueia lista)
  - [ ] Após acção com sucesso: remove a row da lista (optimistic update ou re-fetch)
- [ ] 7.6 Se lista vazia: `<EmptyState>` "Sem pedidos pendentes." (UX-DR8)

---

### Task 8: Testes (AC #1–#7)

- [ ] 8.1 `sparta/src/__tests__/lib/actions/data-rights.test.ts` — adicionar 5 testes:
  - [ ] `submitRectificationRequest` sucesso → retorna `submitted:true`
  - [ ] `submitRectificationRequest` sem player → `err not_found`
  - [ ] `submitRectificationRequestByToken` token válido → `submitted:true`
  - [ ] `approveRectification` sucesso → `applied:true` + audit log inserido
  - [ ] `rejectRectification` com reason → `rejected:true`
- [ ] 8.2 `sparta/src/__tests__/app/configuracoes/direitos/retificar/page.test.tsx` — 3 testes:
  - [ ] Renderiza formulário com select de campo
  - [ ] Redirect para `/login` quando não autenticado
  - [ ] `<CalmConfirmation>` após submit com sucesso (mock Server Action)
- [ ] 8.3 `sparta/src/__tests__/app/public/direitos/token/retificar/page.test.tsx` — 2 testes:
  - [ ] Token válido → mostra formulário com nome do menor
  - [ ] Token inválido/expirado → `<EmptyState>`
- [ ] 8.4 **Total: ≥10 novos testes**

---

### Task 9: Verificação final

- [ ] 9.1 `npm run lint` — zero erros novos
- [ ] 9.2 `npm run typecheck` — zero erros
- [ ] 9.3 `npm run test --run` — todos os testes passando, ≥10 novos

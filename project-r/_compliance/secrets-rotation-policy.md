...
4. **Local dev:** Update `project-r/.env.local` SUPABASE_SERVICE_ROLE_KEY value
...
4. **Local dev:** Update `project-r/.env.local` RESEND_API_KEY value
...
2. **Update:** `project-r/.env.local` and Vercel environment variables
...
2. **Update:** `project-r/.env.local` and GitHub Actions secrets (`BACKUP_ENCRYPTION_KEY`)
